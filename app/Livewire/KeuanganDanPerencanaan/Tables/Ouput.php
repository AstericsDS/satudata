...
final class Ouput extends PowerGridComponent
{
    public string $tableName = 'ouputTable';

    public function datasource(): Collection
    {
        $data = Anggaran::where('data_scope', 'output')->get();
        return $data->map(function ($item, $key) {
            return [
                'id' => $item->id,
                'output_program' => $item->nama,
    ...
    {
        return [
            Column::make('Output Program', 'output_program')
                ->searchable()
                ->sortable(),
