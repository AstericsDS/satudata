<tr wire:poll.3s class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
    ...
    {{-- Nama --}}
    <td class="px-6 py-4">
        Anggaran dan Daya Serap
    </td>

    {{-- Status --}}
    <td class="px-6 py-4">
        @if ($sync->status === 'unsynchronized')
            <span class="bg-[#ECD1D1] p-2 rounded-md border-[1px] border-[#B73E3E]/18 text-red-500">
                Belum Sinkron
            </span>
        @elseif ($sync->status === 'synchronized')
            <span class="bg-[#D7EFEA] p-2 rounded-md border-[1px] border-[#006569]/18 text-primary">
                Tersinkron
            </span>
        @elseif ($sync->status === 'synchronizing')
            <span class="bg-[#F4F0D7] p-2 rounded-md border-[1px] border-[#C9A800]/18 text-yellow-600">
                Menyinkronkan
            </span>
    ...
    <td class="px-6 py-4">
        @if($sync->status !== 'unsynchronized')
            <span class="text-gray-800">
                {{ $sync->updated_at->locale('id')->translatedFormat('d F Y (H:i:s)') }}
            </span>
            <p class="text-gray-400">
                {{ $sync->updated_at->locale('id')->diffForHumans() }}
            </p>
        @else
